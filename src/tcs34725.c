...
int write8(I2C_TypeDef *I2Cx, uint8_t reg, uint32_t value)
{
    ...
    return I2C_writeTwoByte(I2Cx, txBuffer, (uint8_t)(TCS34725_I2C_ADDR << 1));
}

int read8(I2C_TypeDef *I2Cx, uint8_t reg, uint8_t *value)
{
    uint8_t cmd = (uint8_t)(TCS34725_COMMAND_BIT | reg);
    if (!I2C_writeByte(I2Cx, cmd, (uint8_t)(TCS34725_I2C_ADDR << 1))) return 0;
    if (!I2C_readByte(I2Cx, (uint8_t)(TCS34725_I2C_ADDR << 1), value)) return 0;
    return 1;
}
...
int read16(I2C_TypeDef *I2Cx, uint8_t reg, uint16_t *ret)
{
    ...
    uint8_t rxBuffer[2];
    if (!I2C_writeByte(I2Cx, cmd, (uint8_t)(TCS34725_I2C_ADDR << 1))) return 0;
    if (!I2C_readTwoByte(I2Cx, (uint8_t)(TCS34725_I2C_ADDR << 1), rxBuffer)) return 0;
...
void tcs3272_init(I2C_TypeDef *I2Cx)
{
    ...
    if (!setIntegrationTime(I2Cx, 0xD5)) {
